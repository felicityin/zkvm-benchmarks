...
pub fn benchmark_sha3_precompile_chain(iters: u32) -> (Duration, usize, u64) {
    let client = ProverClient::cpu();
    let (pk, vk) = client.setup(SHA3_PRECOMPILE_CHAIN_ELF);
    ...
    // Execute the program using the `ProverClient.execute` method, without generating a proof.
    let (_, report) = client.execute(SHA3_PRECOMPILE_CHAIN_ELF, stdin).run().unwrap();
    println!("executed program with {} cycles", report.total_instruction_count());
...
pub fn benchmark_sha3_precompile(num_bytes: usize) -> (Duration, usize, u64) {
    let client = ProverClient::cpu();
